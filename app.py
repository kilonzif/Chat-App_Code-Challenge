...
# Required to use Flask sessions. - REMOVE
# app.secret_key = "ABC"
...
# curl -k -H "Content-Type: application/json" -X POST -d '{"username":"foo","password":"bar"}' http://localhost:5000/log_in
...
# curl -k -i -H "Accept: application/json" -H "Content-Type: application/json" -X GET http://localhost:5000/log_out
...
# curl -k -H "Content-Type: application/json" -X POST -d '{"sender_id":"1","recipient_id":"2","message_content":"hello world"}' http://localhost:5000/messages
...
def send_message():
    ...
    if message['message_content'].startswith('http'):
        ...
        if helper.is_image(message['message_content']):
            ...
            message_metadata = flask.jsonify({'img_width': '300',  # Image width is stored in px
        ...
        # & vid_length from APIs for whitelisted video platforms
            ...
            message_metadata = flask.jsonify({'vid_length': '180',  # Video length is stored in seconds
                                              'vid_source': 'youtube'})

    elif flask.request.headers['Content-Type'] == 'text/plain':
        message_type = 'text'
        messages_metadata = flask.jsonify({})

    else:
        return unsupported_media_type()

    new_message = modelMessage(conversation_id=conversation_id,
